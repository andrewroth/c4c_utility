require "#{File.dirname(__FILE__)}/../../vendor/plugins/moonshine_multisite/lib/moonshine.rb"
class ApplicationManifest < Moonshine::Manifest::Rails
  ...
  def certs
    ...
    file '/etc/apache2/certs', :ensure => :directory
    for type in %w(cert key)
      dest = "/etc/apache2/certs/#{configuration[:domain]}.#{type}"
      source = "/tmp/moonshine_config_files/#{configuration[:domain]}.#{type}"
